...
INPUT_JSON = "/home/users/ntu/bhargavi/scratch/pure_bird_manifests/test_A.json"
# Output: Final dataset directory
OUTPUT_ROOT = "/home/users/ntu/bhargavi/scratch/test_dataset_top20" # Modify output path to avoid overwriting
...
TARGET_DEV_HOURS = 10.0     # Dev set target: 50 hours
        ...
        while current_time < MIN_SESSION_DURATION:
            ...
            if clip.get('detected_events'):
                for e in clip['detected_events']:
                    ...
                    speech.append([audio_start + e[0],audio_start + e[1]]);
                ...
                speech.append([audio_start,audio_start + audio_dur]);
...
def generate_dataset_partition(split_name, clip_pool, species_list, target_hours, output_root):
    ...
    pbar.close()
    out_json = os.path.join(output_root, "metadata_test_A.json")
    with open(out_json, 'w') as f:
